  ...
  describe('2. Primary Key & Unique Constraints', () => {
    it('creates a valid employee successfully', async () => {
      await executeQuery(
        ...
        [testEmpId, testEmpId, 'Integrity', 'Test Employee', testEmpEmail, 'Engineering', 'QA Engineer', 'ACTIVE']
      );
      ...
      try {
        await assert.rejects(async () => {
          await conn.query(
            ...
            [testEmpId, `${testEmpId}-dup`, 'Duplicate', 'PK Employee', `other.${testSuffix}@example.com`, 'Engineering', 'QA', 'ACTIVE']
          );
    ...
    it('rejects duplicate employee email (UNIQUE email constraint)', async () => {
      ...
      try {
        await assert.rejects(async () => {
          await conn.query(
            'INSERT INTO employees (id, empCode, firstName, lastName, email, department, jobPosition, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
            [`${testEmpId}-DIFF`, `${testEmpId}-DIFF`, 'Duplicate', 'Email Employee', testEmpEmail, 'Engineering', 'QA', 'ACTIVE']
          );
        }, (err: any) => err.code === 'ER_DUP_ENTRY');
      } finally {
        conn.release();
