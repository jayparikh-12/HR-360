 ...
 * Phase 4.8: Percentage-Based Rules.
 ...
 * Phase 4.13: Gross Salary Integration.
 * Phase 4.14: Total Deductions Integration.
 ...
 * Supports both domain classifications and schema codes.
 */
export function isEarningCategory(category: string): boolean {
  const norm = (category || '').trim().toUpperCase();
  return norm === 'BASIC' || norm === 'ALLOWANCE' || norm === 'EARNING' || norm === 'EARNINGS';
}
...
export function isDeductionCategory(category: string): boolean {
  const norm = (category || '').trim().toUpperCase();
  return norm === 'DEDUCTION' || norm === 'DEDUCTIONS';
}
...
export function calculateEarnings(
  rules: (NormalizedSalaryRuleInput | PayrollSalaryRule)[],
  baseWage: number,
  structureId?: string | null
): { totalEarnings: number; earnings: SalaryRuleContribution[] } {
  const result = processSalaryRules(rules, baseWage, structureId);
  return {
...
export function processSalaryRules(
  ...
  const applicable = rules.filter((r) => {
    ...
    const ruleStruct = r.structureId || (r as any).structure_id || (r as any).salaryStructureId;
    if (structureId && ruleStruct && ruleStruct !== structureId) return false;
  ...
  applicable.sort((a, b) => {
    ...
    const idA = 'ruleId' in a ? a.ruleId : a.id;
    const idB = 'ruleId' in b ? b.ruleId : b.id;
  ...
  for (const rule of applicable) {
    ...
    const ruleId = 'ruleId' in rule ? rule.ruleId : rule.id;
    const calcType = (rule.calculationType || (rule as any).calculation_type || 'FIXED') as SalaryRuleCalculationType;
    const category = (rule.category.toUpperCase() as SalaryRuleCategory);
    const sequence = typeof rule.sequence === 'number' ? rule.sequence : parseInt(String(rule.sequence), 10) || 0;
...
export class PayrollEngine {
  ...
  public static calculateRuleContribution = calculateSalaryRuleContribution;
