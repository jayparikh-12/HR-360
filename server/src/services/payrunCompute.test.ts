...
describe('PHASE 5.2 — Payrun Compute Workflow', () => {
  ...
  before(async () => {
    ...
    await executeQuery(
      `INSERT INTO employees (id, empCode, firstName, lastName, email, department, jobPosition, status, createdAt, bankAccountNo)
       VALUES (?, ?, 'Terminated', 'User', ?, 'Engineering', 'Former Engineer', 'INACTIVE', '2026-01-01', '•••• 9999')`,
      [terminatedEmpId, terminatedEmpId, `term_${testSuffix}@company.com`]
    );
    await executeQuery(
    ...
    await executeQuery(
      `INSERT INTO employees (id, empCode, firstName, lastName, email, department, jobPosition, status, createdAt, bankAccountNo)
       VALUES (?, ?, 'Future', 'Hire', ?, 'Product', 'Future PM', 'ACTIVE', '2026-01-01', '•••• 8888')`,
      [futureContractEmpId, futureContractEmpId, `future_${testSuffix}@company.com`]
    );
    await executeQuery(
      `INSERT INTO contracts (id, employee_id, salary_structure_id, working_schedule_id, wage, start_date, status)
